...
const ForestSceneNight = () => {
  ...
  useEffect(() => {
    ...
    const directionalLight = new THREE.DirectionalLight(0xffffff, 0.5); //intensitate
    directionalLight.position.set(10, 10, 10).normalize();
    scene.add(directionalLight);

    const ambientLight = new THREE.AmbientLight(0x111111, 0.2); //intensitate
  ...
  return (
    <div ref={sceneRef} style={{ width: '100%', height: '100vh' }}>
      {scene && (
        <>
          <Ground scene={scene} />
          <StarField scene={scene} /> {/* Folosim noua componentă StarField */}
          <Plane scene={scene} />
          <House scene={scene} />
          ...
          <Fire scene={scene} position={[-15, 0, 30]} scale={[2, 2, 2]} /> {/* Adăugăm modelul de foc */}
        </>
      )}
    </div>
  );
...
export default ForestSceneNight;
